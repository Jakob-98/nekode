import XCTest
@testable import CatAssistant
import SwiftUI

/// QA snapshot tests that render PopupView with various session configurations.
///
/// Each test saves a PNG to /tmp/catassistant-qa/<scenario>.png for visual inspection.
/// Run all QA snapshots:
///   xcodebuild test -project menubar/CctopMenubar.xcodeproj -scheme CatAssistant \
...
final class QASnapshotTests: XCTestCase {
    ...
    func testAddFifthSession() throws {
        ...
        try captureToFile(hostingView: hostingView, path: "/tmp/catassistant-qa/11-add-fifth-before.png")
        ...
        hostingView.frame = NSRect(origin: .zero, size: fittingSize)
        hostingView.layoutSubtreeIfNeeded()
        try captureToFile(hostingView: hostingView, path: "/tmp/catassistant-qa/11-add-fifth-after.png")
    }
    ...
    private func renderSnapshot(
        ...
        try captureToFile(hostingView: hostingView, path: "/tmp/catassistant-qa/\(name).png")
    }
