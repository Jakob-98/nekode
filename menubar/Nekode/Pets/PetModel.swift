...
class PetModel: ObservableObject, Identifiable {
    ...
    /// When true, the pet is actively running back to the vibe zone
    /// (triggered by the "Run to Vibe Zone" button). Uses runInSpeed
    /// instead of the normal vibeWanderSpeed, and clears on arrival.
    var isReturningToZone: Bool = false
    ...
    var attentionStage: Int {
        if attentionTime < PetPhysics.attentionStage2Threshold { return 1 }     // Perk up (stay put, face cursor)
        if attentionTime < PetPhysics.attentionStage3Threshold { return 2 }    // Approach (walk toward, 50px stop)
        if attentionTime < PetPhysics.attentionStage4Threshold { return 3 }    // Insistent (faster, 30px stop)
    ...
    var idleTier: Int {
        if idleTime < PetPhysics.idleTier1Threshold { return 0 }      // Just sat down (0-5s)
        if idleTime < PetPhysics.idleTier2Threshold { return 1 }     // Attentive (5-30s)
        if idleTime < PetPhysics.idleTier3Threshold { return 2 }    // Bored (30-120s)
    ...
    var bubbleCooldown: Double = PetPhysics.bubbleInitialCooldown
    ...
    var visualState: PetState {
        ...
        // Returning to vibe zone at run speed → running animation
        if state == .sitting && isReturningToZone && speed > PetPhysics.velocitySnapThreshold {
            return .running
        }
    ...
    init(session: Session, kind: PetKind, screenBounds: CGRect, vibeZone: CGRect? = nil) {
        ...
        // If the session already needs attention at spawn time, treat it as
        // acknowledged so the pet doesn't immediately start chasing the cursor.
        // This prevents toggling pets off/on from resetting attention states.
        let initialState = PetState(from: session.status)
        if initialState.isAttentionSeeking {
            self.state = .sitting
            self.dismissedStatus = session.status
        } else {
