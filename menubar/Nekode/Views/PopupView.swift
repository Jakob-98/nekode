...
extension Notification.Name {
    ...
    static let vibeZoneMoved = Notification.Name("vibeZoneMoved")
}
            ...
            HStack(spacing: 8) {
                Toggle(isOn: $desktopPetsEnabled) {
                    Label("Cats", systemImage: "pawprint.fill")
                        ...
                        .foregroundStyle(desktopPetsEnabled ? Color.amber : Color.textMuted)
                }
                .toggleStyle(.switch)
                .controlSize(.mini)
                .labelsHidden()
                ...
                if desktopPetsEnabled {
                    Button {
                        petManager?.returnAllPetsToVibeZone()
                    } label: {
                        HStack(spacing: 3) {
                            Image(systemName: "arrow.uturn.backward")
                                .font(.system(size: 9))
                            Text("Run to Vibe Zone")
                                .font(.system(size: 10, weight: .medium))
                        }
