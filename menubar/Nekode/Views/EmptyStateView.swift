...
struct EmptyStateView: View {
    ...
    private static let ccMarketplace = "claude plugin marketplace add Jakob-98/nekode"
    private static let ccInstall = "claude plugin install nekode"

    private var anyInstalled: Bool {
        pluginManager.ccInstalled || pluginManager.ocInstalled || pluginManager.copilotInstalled || pluginManager.copilotCLIInstalled
    }
    ...
    private var copilotCLIPluginRow: some View {
        VStack(spacing: 4) {
            HStack(spacing: 6) {
                pluginStatusRow("Copilot CLI", installed: pluginManager.copilotCLIInstalled)
                if !pluginManager.copilotCLIInstalled && !copilotCLIJustInstalled {
                    Button {
            ...
            if copilotCLIJustInstalled {
                HStack(spacing: 4) {
                    Image(systemName: "checkmark")
                        .font(.system(size: 10))
                        .foregroundStyle(.green)
                    Text("Connected \u{2014} restart Copilot CLI to activate")
                        .font(.system(size: 10))
                        .foregroundStyle(Color.textMuted)
                    Spacer()
                }
