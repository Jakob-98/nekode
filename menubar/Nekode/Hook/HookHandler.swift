...
enum HookHandler {
    ...
    static func handleHook(hookName: String, input: HookInput, source: SessionSource? = nil) throws {
        ...
        let newStatus = Transition.forEvent(event, source: session.source)
    ...
    static func cleanupSessionsForProject(sessionsDir: String, projectPath: String, currentPid: UInt32?) {
        forEachSession(in: sessionsDir) { path, session in
            ...
            if let pid = session.pid {
                ...
                isStale = true  // No PID means legacy session — always stale
            }
