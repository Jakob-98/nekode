...
private let logger = Logger(subsystem: "dev.nekode.Nekode", category: "SessionManager")
...
class SessionManager: ObservableObject {
    ...
    func loadSessions() {
        ...
        let newSessions = alive.map(\.1)
        // Only assign if changed — avoids unnecessary SwiftUI redraws
        if newSessions.map(\.id) != sessions.map(\.id)
            || newSessions.map(\.status) != sessions.map(\.status)
            || newSessions.map(\.lastActivity) != sessions.map(\.lastActivity) {
