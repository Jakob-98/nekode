...
func focusTerminal(session: Session) {
    guard let terminal = session.terminal else {
        // No terminal info — try activating frontmost terminal app, else open Finder
        if !activateFrontmostTerminal() {
    ...
    } else if !activateFrontmostTerminal() {
        // Last resort: try any running terminal app, else open in Finder
        NSWorkspace.shared.open(URL(fileURLWithPath: session.projectPath))
    }
}

/// Try to activate the first running terminal emulator we recognize.
/// Returns true if a terminal app was found and activated.
@discardableResult
private func activateFrontmostTerminal() -> Bool {
