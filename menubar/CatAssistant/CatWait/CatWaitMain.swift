...
/// CLI entry point for catwait.
///
/// Pipe any long-running command through catwait to monitor it as a CatAssistant session.
...
///
/// Usage: some_command | catwait [--name <name>] [--project <path>]
@main
struct PetWaitMain {
    ...
    static func main() {
        ...
        while idx < args.count {
            ...
            case "--name":
                guard idx + 1 < args.count else {
                    FileHandle.standardError.write(
                        Data("catwait: --name requires a value\n".utf8))
                    exit(1)
                }
                idx += 1
                name = args[idx]
            case "--project":
                guard idx + 1 < args.count else {
                    FileHandle.standardError.write(
                        Data("catwait: --project requires a value\n".utf8))
                    exit(1)
                }
                idx += 1
            ...
            default:
                FileHandle.standardError.write(
                    Data("catwait: unknown option '\(args[idx])'\n".utf8))
                printHelp()
                exit(1)
            }
            idx += 1
        ...
        do {
            ...
            FileHandle.standardError.write(
                Data("catwait: failed to write session: \(error)\n".utf8))
            exit(1)
        }
        ...
        FileHandle.standardError.write(
            Data("\ncatwait: command finished. Press Enter to dismiss.\n".utf8))
    ...
    /// Detect the terminal app hosting this catwait process via environment
    /// variables, mirroring cathook's captureTerminalInfo().
    ...
    private static func printHelp() {
        print("catwait \(version)")
        print("Pipe-based session monitoring for CatAssistant.\n")
        print("Pipe any long-running command through catwait to track it")
        print("as a live session in the CatAssistant menubar and desktop pets.\n")
        print("USAGE:")
        print("    some_command | catwait [OPTIONS]\n")
        print("EXAMPLES:")
        print("    cargo build --release 2>&1 | catwait")
        print("    npm run build | catwait --name \"npm build\"")
        print(
            "    make test 2>&1 | catwait --name \"tests\" --project ~/myapp\n")
        print("OPTIONS:")
