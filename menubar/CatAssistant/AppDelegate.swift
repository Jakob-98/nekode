...
class AppDelegate: NSObject, NSApplicationDelegate, UNUserNotificationCenterDelegate {
    ...
    func applicationDidFinishLaunching(_ notification: Notification) {
        UserDefaults.standard.register(defaults: ["notificationsEnabled": true, "desktopPetsEnabled": true])
        ...
        sessionManager.$sessions
            ...
            .sink { [weak self] sessions in
                ...
                let a11yLabel = count > 0
                    ? "CatAssistant, \(count) session\(count == 1 ? "" : "s") need attention"
                    : "CatAssistant, \(sessions.count) session\(sessions.count == 1 ? "" : "s")"
...
extension AppDelegate {
    /// Symlinks cathook from the app bundle into ~/.cat/bin/ so hooks can find it.
    func installHookBinaryIfNeeded() {
        let fm = FileManager.default
        let home = fm.homeDirectoryForCurrentUser
        guard let bundledHook = Bundle.main.url(forAuxiliaryExecutable: "cathook") else { return }
        let catBin = home.appendingPathComponent(".cat/bin")
        let symlinkPath = catBin.appendingPathComponent("cathook")
        if let dest = try? fm.destinationOfSymbolicLink(atPath: symlinkPath.path),
           URL(fileURLWithPath: dest) == bundledHook { return }
        do {
            try fm.createDirectory(at: catBin, withIntermediateDirectories: true)
