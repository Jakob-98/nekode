...
final class PetStateTests: XCTestCase {
    ...
    func testAllCasesCount() {
        XCTAssertEqual(PetState.allCases.count, 17)
    }
...
final class PetModelTests: XCTestCase {
    ...
    func testSpeechBubbleForAttentionStates() {
        // At init, attention states are suppressed (treated as dismissed)
        // so pets don't immediately chase cursor on spawn. speechBubble
        // only appears after the animation engine transitions the pet.
        let barkingPet = makePet(status: .waitingPermission)
        XCTAssertNil(barkingPet.speechBubble)

        let alertingPet = makePet(status: .waitingInput)
        XCTAssertNil(alertingPet.speechBubble)

        // Verify speech bubble works when state is set directly
        let pet = makePet(status: .idle)
        pet.state = .barking
        XCTAssertEqual(pet.speechBubble, "!")
        pet.state = .alerting
        XCTAssertEqual(pet.speechBubble, "?")
    }
...
final class PetAnimationEngineTests: XCTestCase {
    ...
    func testDraggingPreventsMovement() {
        ...
        finishAppearing(pet)
        pet.state = .walking
        pet.velocity = CGPoint(x: PetPhysics.idleWalkSpeed, y: 0)
        pet.isDragging = true
...
final class PetPhysicsTests: XCTestCase {
    ...
    func testConstantsArePositive() {
        XCTAssertGreaterThan(PetPhysics.idleWalkSpeed, 0)
    ...
    func testAttentionSpeedGreaterThanIdleWalkSpeed() {
        XCTAssertGreaterThan(PetPhysics.attentionSpeed, PetPhysics.idleWalkSpeed)
    }
}
