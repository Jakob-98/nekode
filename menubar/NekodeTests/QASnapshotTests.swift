...
///
/// Each test saves a PNG to /tmp/nekode-qa/<scenario>.png for visual inspection.
/// Run all QA snapshots:
///   xcodebuild test -project menubar/Nekode.xcodeproj -scheme Nekode \
///     -only-testing:NekodeTests/QASnapshotTests \
...
final class QASnapshotTests: XCTestCase {
    ...
    func testAddFifthSession() throws {
        ...
        try captureToFile(hostingView: hostingView, path: "/tmp/nekode-qa/11-add-fifth-before.png")
        ...
        hostingView.frame = NSRect(origin: .zero, size: fittingSize)
        hostingView.layoutSubtreeIfNeeded()
        try captureToFile(hostingView: hostingView, path: "/tmp/nekode-qa/11-add-fifth-after.png")
    }
    ...
    private func renderSnapshot(
        ...
        try captureToFile(hostingView: hostingView, path: "/tmp/nekode-qa/\(name).png")
    }
