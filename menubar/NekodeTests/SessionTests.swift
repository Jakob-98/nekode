...
final class SessionTests: XCTestCase {
    ...
    func testSourceLabelUnknownDecodesToDefault() throws {
        // Unknown source strings in JSON decode as nil (SessionSource? is optional)
        let json = """
        {
            "session_id": "unknown-src",
            "project_path": "/tmp",
            "project_name": "test",
            "branch": "main",
            "status": "idle",
        ...
        """
        let session = try JSONDecoder.sessionDecoder.decode(Session.self, from: Data(json.utf8))
        // Unknown source raw value decodes as nil → sourceLabel defaults to "CC"
        XCTAssertNil(session.source)
        XCTAssertEqual(session.sourceLabel, "CC")
    }
