...
final class ForkSessionTests: XCTestCase {
    ...
    override func setUp() {
        ...
        sessionsDir = NSTemporaryDirectory()
            + "nekode-fork-test-\(UUID().uuidString)/"
        try! FileManager.default.createDirectory(
            atPath: sessionsDir,
            withIntermediateDirectories: true
        )

        // Prefer Xcode's BUILT_PRODUCTS_DIR (reliable in CI),
        // fall back to #file-relative path for local runs.
    ...
    private func fireSessionStart(
        ...
        guard FileManager.default.fileExists(atPath: hookBinary)
            ...
            XCTFail(
                "nekode not found at \(hookBinary!)."
        ...
        process.arguments = ["hook", "SessionStart"]
        process.environment = [
            "NEKODE_SESSIONS_DIR": sessionsDir,
            "TERM_PROGRAM": "Test",
        ]
