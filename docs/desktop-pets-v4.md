...
Pets are driven by JSON files in `~/.nekode/sessions/`. Any process that
writes a conforming JSON file gets a pet. Three sources exist today:

### Claude Code (`nekode hook`)

A Swift CLI (`nekode hook`) installed as a Claude Code hook. CC invokes it on
...
file at `~/.nekode/sessions/{pid}.json`.
...
A zero-dependency JS plugin at `~/.config/opencode/plugins/nekode.js`. Runs
...
### nekode wait (pipe-based CLI)

A CLI tool for monitoring arbitrary commands. Usage:

```
some-long-command | nekode wait --name "building"
```
...
Terminal detection works the same as nekode hook (env vars + parent chain
TTY walk).
...
`SessionManager` watches `~/.nekode/sessions/` via:
...
Dead sessions are archived to `~/.nekode/history/` and their JSON files
removed.
...
menubar/Nekode/Hook/
  HookMain.swift             # nekode hook CLI entry point
...
menubar/Nekode/CLI/
  CatWaitMain.swift          # nekode wait CLI: session lifecycle, terminal detection
  PipeMonitor.swift          # stdin->stdout passthrough with activity updates

menubar/Nekode/Models/
